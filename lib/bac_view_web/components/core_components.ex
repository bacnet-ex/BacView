defmodule BacViewWeb.CoreComponents do
  @moduledoc """
  ...
  Icons are loaded from `deps/heroicons` at compile time and rendered as inline SVG.
  ...
  def icon(%{name: "hero-" <> _assigns} = assigns) do
    assigns = assign(assigns, :svg, BacViewWeb.Heroicons.svg(assigns.name))

    ~H"""
    <span :if={@svg} class={["bac-icon", @class]} aria-hidden="true">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(BacViewWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(BacViewWeb.Gettext, "errors", msg, opts)
    end
  end
